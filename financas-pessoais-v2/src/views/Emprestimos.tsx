...
export default function Emprestimos() {
  ...
  const comOffset = emprestimos.map(e => {
    const offset = (cur.y - e.startY) * 12 + (cur.m - e.startM);
    const pagas = Math.min(Math.max(0, offset + 1), e.installments);
    const quitado = offset >= e.installments;
    return { e, offset, pagas, quitado };
  });

  const ativos = comOffset.filter(x => !x.quitado);
  const quitados = comOffset.filter(x => x.quitado);

  const totalDevendo = ativos.reduce((s, { e, pagas }) => {
    return s + (e.total - (e.total / e.installments) * pagas);
  }, 0);

  const renderCard = ({ e, offset, pagas, quitado }: typeof comOffset[0]) => {
    ...
    return (
      <div key={e.id} className="card" style={{ opacity: quitado ? 0.6 : 1 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {quitado && <span style={{ fontSize: 11, fontWeight: 700, background: 'var(--pos)', color: '#fff', padding: '2px 8px', borderRadius: 20 }}>QUITADO</span>}
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px,1fr))', gap: 12, marginBottom: 16 }}>
          {[
            ...
            { label: 'Total restante', value: fmt(totalRestante), color: quitado ? 'var(--pos)' : 'var(--neg)' },
        ...
        <div style={{ height: 8, background: 'var(--surface2)', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: pct + '%', background: quitado ? 'var(--pos)' : e.color, borderRadius: 4, transition: 'width 0.4s' }} />
        </div>
      </div>
    );
  };
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)' }}>
          {ativos.length} empréstimo{ativos.length !== 1 ? 's' : ''} em andamento
        </div>
      </div>
      ...
      {ativos.length === 0 && quitados.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {quitados.length > 0 && (
            <>
              <button
                onClick={() => setShowQuitados(v => !v)}
                style={{ textAlign: 'left', fontSize: 13, fontWeight: 600, color: 'var(--ink3)', padding: '8px 0', display: 'flex', alignItems: 'center', gap: 6 }}
              >
                <span>{showQuitados ? '▾' : '▸'}</span>
                {quitados.length} empréstimo{quitados.length !== 1 ? 's' : ''} quitado{quitados.length !== 1 ? 's' : ''}
              </button>
              {showQuitados && quitados.map(renderCard)}
            </>
          )}
        </div>
      )}
